...
async def _query_api(url, resp_obj):
    ...
    if rate_limiter is None:
        ...
        rate_limiter = RateLimiter(rate_limit=5, concurrency_limit=3)
...
class User:
    def __init__(self, data):
        ...
        self.raw_rating = None
        self.max_raw_rating = None
    ...
    async def async_init(self):
        ...
        for contest in self._contests:
            if contest['rating']:
                self.max_rating = max(self.max_rating or -1e9, contest['rating'])
                self.raw_rating = contest['raw_rating'] or self.raw_rating
                self.max_raw_rating = max(self.max_raw_rating or -1e9, contest['raw_rating'])

...
from utils.db import session, User as User_DB
...
async def rate_contest(contest):
    ...
    ranks = list(tie_ranker(users, key=itemgetter('score', 'cumulative_time', 'tiebreaker')))
    old_ratings = []
    raw_ratings = []
    counts = []
    perfs = []
    ...
    rating, new_raw, performance = recalculate_ratings(ranks, raw_ratings, counts, perfs)
    predictions = {}
    for name, a, b, c, d in zip(usernames, ranks, old_ratings, rating, performance):
        predictions[name] = {
            'rank': a,
            'old_rating': b,
            'new_rating': c,

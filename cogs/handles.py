...
class Handles(commands.Cog):
    ...
    @commands.command(aliases=['users', 'leaderboard'], usage='[rating|maxrating|points|solved|raw|maxraw]')
    async def top(self, ctx, arg='rating'):
        '''Shows registered server members in ranked order'''
        arg = arg.lower()
        if arg not in ['rating', 'maxrating', 'points', 'solved', 'raw', 'maxraw']:
        ...
        for user in users:
            if arg == 'rating':
                ...
                leaderboard.append([-(user.raw_rating or -9999), user.username])
            elif arg == 'maxraw':
                leaderboard.append([-(user.max_raw_rating or -9999), user.username])
        leaderboard.sort()
        content = []
        page = ''

...
class User(commands.Cog):
    ...
    async def userinfo(self, ctx, username: typing.Optional[str_not_int] = None,
        ...
        embed.add_field(
            name='Rating',
            value=f"{user.rating}/{user.max_rating}({round(user.raw_rating)}/{round(user.max_raw_rating)})"
            if user.rating else "-",
            inline=True
        )
        embed.add_field(
    ...
    async def solved(self, ctx, *args):
        ...
        for sub in uniqueSubmissions:
            ...
            newRow = f'[{sub.problem[0].name}]({SITE_URL}/problem/{sub._code}) [{sub.points}] ({age} days ago)\n'
            if cnt > 0 and cnt % 10 == 0 or len(page) + len(newRow) > 1024:
                content.append(page)
                page = ''
            cnt += 1
            page += newRow
